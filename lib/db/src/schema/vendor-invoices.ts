...
export const vendorInvoicesTable = pgTable("vendor_invoices", {
  ...
  piDate: text("pi_date"),
  paymentTerms: text("payment_terms").notNull().default("30 Days Net"),
  dueDate: text("due_date"),
  plannedPaymentDate: text("planned_payment_date"),
  remindersEnabled: boolean("reminders_enabled").notNull().default(false),
  reminderStartAfterDay: integer("reminder_start_after_day"),
  reminderEmails: jsonb("reminder_emails").notNull().default([]),
