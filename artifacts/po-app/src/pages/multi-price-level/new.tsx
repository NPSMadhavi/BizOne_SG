  ...
  return (
    <div className="min-h-full space-y-5 bg-[#f6f8fc] p-1 animate-in fade-in duration-500">
      ...
      <Card><CardHeader className="border-b py-4"><CardTitle className="flex items-center gap-2 text-sm"><Settings2 className="h-4 w-4 text-[#1265d8]" /> Basic Information</CardTitle></CardHeader><CardContent className="grid gap-5 p-5 lg:grid-cols-[1.05fr_0.8fr_1.4fr_0.8fr]">
        <div className="space-y-1.5"><Label>Price Level Name <span className="text-destructive">*</span></Label><Input placeholder="Enter price level name" value={name} onChange={e => setName(e.target.value)} /><p className="text-[11px] text-muted-foreground">Example: Retail Price, Wholesale Price, Dealer Price, Distributor Price</p></div>
        <div className="space-y-1.5"><Label>Code <span className="text-destructive">*</span></Label><Input placeholder="Enter code" value={code} onChange={e => setCode(e.target.value.toUpperCase())} /><p className="text-[11px] text-muted-foreground">Unique code for this price level</p></div>
        <div className="space-y-1.5"><Label>Description</Label><Textarea placeholder="Enter description (optional)" maxLength={200} value={description} onChange={e => setDescription(e.target.value)} rows={3} /><p className="text-right text-[11px] text-muted-foreground">{description.length}/200</p></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.55fr_1fr]">
        <Card><CardHeader className="border-b py-4"><CardTitle className="flex items-center gap-2 text-sm"><Settings2 className="h-4 w-4 text-[#1265d8]" /> Price Level Settings</CardTitle></CardHeader><CardContent className="grid gap-5 p-5 sm:grid-cols-2 xl:grid-cols-3">
          <div className="space-y-1.5"><Label>Priority Order <span className="text-destructive">*</span></Label><Input type="text" inputMode="numeric" pattern="[0-9]*" value={priority} onChange={e => setPriority(e.target.value.replace(/\D/g, ""))} placeholder="Enter priority order" /><p className="text-[11px] text-muted-foreground">Lower number = Higher priority</p></div>
