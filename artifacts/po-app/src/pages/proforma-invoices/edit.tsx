...
export default function ProformaInvoiceEdit() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { items: [{ type: "item", sectionLabel: "", sectionAlign: "left", partNumber: "", description: "", qty: 1, uom: "", unitPrice: 0, discount: 0, isFoc: false, itemImage: "" }] },
  });
  ...
  useEffect(() => {
    if (doc && !formLoaded) {
      const rawItems = (doc.items as any[]) || [];
      form.reset({
        ...
        tax: (() => {
          ...
          return taxable > 0 ? Math.round((taxAmt / taxable) * 10000) / 100 : Number(settings?.gstRate ?? 9);
        })(),
        ...
        })) : [{ type: "item", sectionLabel: "", sectionAlign: "left", partNumber: "", description: "", qty: 1, uom: "", unitPrice: 0, discount: 0, isFoc: false, itemImage: "" }],
      });
      setFormLoaded(true);
    }
  }, [doc, formLoaded, form, settings]);
  ...
  const buildBody = (values: FormValues) => {
    ...
    return { ...values, subtotal, tax: taxAmount, totalAmount: total };
  };
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      if (previewModeRef.current === "preview") {
        ...
        setPreviewDoc({ ...updated, tax: values.tax });
        setPreviewOpen(true);
      } else {
        await updateMutation.mutateAsync(body);
        toast({ title: "Proforma Invoice saved." });
        setLocation(`/proforma-invoices/${id}`);
      }
    } catch (err: any) {
  ...
  if (isLoading || !formLoaded) return (
    <div className="space-y-6"><Skeleton className="h-8 w-48" /><Skeleton className="h-64 w-full" /></div>
  );

  return (
    <div className="max-w-[1600px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => setLocation(`/proforma-invoices/${id}`)}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">Edit {doc?.piNumber}</h1>
                            ...
                            <td colSpan={7} className="px-3 py-2">
                              <div className="flex items-center gap-2">
                                <FormField control={form.control} name={`items.${index}.sectionLabel`} render={({ field }) => (
                                  <FormItem className="flex-1 mb-0"><FormControl><RichTextEditor value={field.value || ""} onChange={field.onChange} placeholder="Section heading..." className="text-sm font-semibold" /></FormControl></FormItem>
                                )} />
          ...
          <Card>
            ...
            <CardContent>
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem><FormControl><Textarea {...field} rows={3} placeholder="Additional notes..." /></FormControl><FormMessage /></FormItem>
              )} />
            </CardContent>
          </Card>
