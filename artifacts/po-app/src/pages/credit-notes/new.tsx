...
export default function CreditNoteNew() {
  ...
  const nextCnNumber = (() => {
    ...
    const prefix = (settings as any).cnPrefix ?? "CN";
    const counter = (parseInt((settings as any).cnCounter) || 0) + 1;
    const suffix = (settings as any).cnSuffix ?? "";
        ...
        <div className="flex items-center gap-4">
          {nextCnNumber && (
            <div className="text-right">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Credit Note Number</p>
                        ...
                        <tr key={field.id} className={cn("border-b border-gray-100", isSection ? "bg-gray-50" : "")}>
                          ...
                          {isSection ? (
                            ...
                            <>
                              ...
                              <td className="px-3 py-2">
                                <Input {...form.register(`items.${idx}.unitPrice`, { onChange: () => updateItemAmount(idx) })} type="text" inputMode="decimal" min={0} step={0.01} className="text-xs h-8 text-right w-28 ml-auto" />
                              </td>
                              <td className="px-3 py-2">
                                <Input {...form.register(`items.${idx}.discount`, { onChange: () => updateItemAmount(idx) })} type="text" inputMode="decimal" min={0} max={100} step={0.01} className="text-xs h-8 text-right w-20 ml-auto" />
                              </td>
