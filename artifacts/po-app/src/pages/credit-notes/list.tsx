...
export default function CreditNoteList() {
  ...
  const { data = [], isLoading } = useQuery<CreditNote[]>({
    ...
    queryFn: async () => {
      const r = await fetch("/api/credit-notes", { credentials: "include" });
  ...
  const getPartyName = (d: CreditNote) => d.customerName || "";
  const bulk = useBulkPartyEmail<CreditNote>({ allDocs: data, dateFiltered: filteredByDate, getPartyName });
  ...
  return (
    <div className="space-y-5 pb-20 animate-in fade-in duration-300">
      ...
      <BulkEmailBar
        ...
        searchPlaceholder="Search CN number, customer…"
        partyLabel="Customer"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-gray-200 bg-gray-50">
                <BulkSelectHeader allSelected={allSelected} someSelected={someSelected} disabled={sendable.length === 0} onToggle={(checked) => bulk.toggleSelectAll(filtered, checked)} label="Select all credit notes" />
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={9} className="text-center py-16 text-gray-400">
                    {search || filterMode !== "all" || bulk.partyFilter !== "all" ? "No credit notes match your filters." : "No credit notes yet. Create your first one."}
                    {search && (
                      <div className="mt-2">
      ...
      <ListBulkEmailDialog
        ...
        docLabel="Credit Notes"
        numbers={bulk.selectedDocs.map(d => d.cnNumber)}
        generateAttachments={async () => {
          const attachments: { filename: string; content: string }[] = [];
          for (const doc of bulk.selectedDocs) {
            const full = await fetchDocJson("credit-notes", doc.id).catch(() => doc);
            const content = await generateCreditNote_PDF({ ...full, items: full.items || [] }, selectedCompany, { returnBase64: true });
            if (typeof content !== "string" || !content) throw new Error(`Could not generate PDF for ${doc.cnNumber}.`);
            attachments.push({ filename: `${doc.cnNumber}.pdf`, content });
          }
          return attachments;
        }}
        onSuccess={async (recipients) => {
          await markDocsSent("credit-notes", bulk.selectedDocs.map(d => d.id), recipients);
