...
import { useCreatePurchaseQuotation, useGetSettings, getGetSettingsQueryKey, getListPurchaseQuotationsQueryKey } from "@workspace/api-client-react";
...
export default function PurchaseQuotationNew() {
  ...
  const nextPqNumber = (() => {
    ...
    const prefix = (settings as any).pqPrefix ?? "";
    const counter = (parseInt((settings as any).pqCounter) || 0) + 1;
    const suffix = (settings as any).pqSuffix ?? "";
  ...
  async function doSubmit(values: z.infer<typeof schema>, openPreview = false) {
    ...
    createMutation.mutate({ data: { ...values, status: openPreview ? "confirmed" : "draft", discountAmount: values.discountAmount, items: itemsWithAmount } as any }, {
      onSuccess: async (data) => {
        // Show the new purchaseQuotation in the list immediately (no manual refresh).
        queryClient.setQueryData(getListPurchaseQuotationsQueryKey(), (old: any) =>
          Array.isArray(old) ? [data, ...old.filter((d: any) => d.id !== (data as any)?.id)] : [data],
        );
        await invalidateDocumentList(queryClient, "purchase-quotations");
        setIsSubmitting(false);
        if (openPreview) {
          setSavedDoc(data);
          setPreviewOpen(true);
      ...
      onError: (err: any) => {
        toast({ title: "Error", description: err?.message || "Failed to create purchase quotation.", variant: "destructive" });
        setIsSubmitting(false);
      },
    });
  }
 ...
 variant="ghost"
 size="icon"
 onClick={() => setLocation("/purchase-quotations")}
 className="h-9 w-9 shrink-0"
          >
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-[#2563EB]">New Purchase Quotation</h1>
            <p className="text-muted-foreground mt-1">Create a new vendor purchase quotation.</p>
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          {nextPqNumber && (
            <div className="text-right">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Purchase Quotation Number</p>
              <p className="text-lg font-semibold font-mono">{nextPqNumber}</p>
            </div>
          )}
        </div>
      </div>
 ...
 onSelect={(c) => {
                    ...
                    if (c.quotationTerms) {
                      ...
                      form.setValue("notes", (settings as any)?.quotationTerms ?? "");
                    }
                  }}
                />
              </CardHeader>
 ...
 onSelect={(c) => {
                ...
                <FormField control={form.control} name="vendorAddress" render={({ field }) => (
            ...
            <Card>
              <CardHeader className="pb-4"><CardTitle className="text-lg">Purchase Quotation Details</CardTitle></CardHeader>
              <CardContent className="space-y-4">
 ...
 title={`Purchase Quotation ${savedDoc.pqNumber}`}
 generatePdf={(opts) => generatePurchaseQuotation_PDF(savedDoc, selectedCompany, settings as any, opts)}
 pdfFilename={`${savedDoc.pqNumber}.pdf`}
 defaultEmailTo={savedDoc.vendorContactEmail || ""}
 defaultEmailSubject={`Purchase Quotation ${savedDoc.pqNumber}`}
 defaultEmailBody={`Dear ${savedDoc.vendorContact || "Sir/Madam"},\n\nPlease find attached our Purchase Quotation ${savedDoc.pqNumber} for your consideration.\n\nDo not hesitate to contact us if you have any questions.\n\nThank you.`}
 docInfo={{
            docType: "Purchase Quotation",
            docNumber: savedDoc.pqNumber,
 ...
 onEmailSent={async (recipients) => {
            await fetch(`/api/purchase-quotations/${savedDoc.id}/mark-sent`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sentTo: recipients }) });
          }}
 onEdit={() => { setPreviewOpen(false); setLocation(`/purchase-quotations/${savedDoc.id}/edit`); }}
        />
      )}
    </div>
  );
