...
import { useGetPurchaseQuotation, useUpdatePurchaseQuotation, getGetPurchaseQuotationQueryKey, useGetSettings, getGetSettingsQueryKey } from "@workspace/api-client-react";
...
export default function PurchaseQuotationEdit() {
  ...
  const { data: doc } = useGetPurchaseQuotation(id, {
  ...
  useEffect(() => {
    if (doc && !initialized.current) {
      ...
      form.reset({
        ...
        vendorContactEmail: (doc as any).vendorContactEmail || "",
      ...
      // restored when the user comes back to edit a purchaseQuotation with a discount.
  ...
  async function doSubmit(values: z.infer<typeof schema>, openPreview = false) {
    ...
    updateMutation.mutate({ id, data: { ...values, status: newStatus, discountAmount: values.discountAmount, items: itemsWithAmount } as any }, {
      onSuccess: async () => {
        await queryClient.refetchQueries({ queryKey: getGetPurchaseQuotationQueryKey(id) });
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => setLocation(`/purchase-quotations/${id}`)}><ArrowLeft className="h-4 w-4" /></Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-[#2563EB]">Edit Purchase Quotation</h1>
            <p className="text-muted-foreground mt-1">{doc.pqNumber}</p>
          </div>
        </div>
      </div>
                ...
                <DirectoryPickerButton
                  ...
                  onSelect={(c) => {
                    ...
                    if (c.quotationTerms) {
                      ...
                      form.setValue("notes", (docSettings as any)?.quotationTerms ?? "");
                    }
                  }}
                />
              </CardHeader>
