  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <BulkEmailBar
        ...
        searchPlaceholder="Search by QT Number or Customer..."
        partyLabel="Customer"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-muted-foreground uppercase bg-muted/50 border-b">
              <tr>
                <BulkSelectHeader allSelected={allSelected} someSelected={someSelected} disabled={sendable.length === 0} onToggle={(checked) => bulk.toggleSelectAll(filtered, checked)} label="Select all quotations" />
              ...
              {isLoading ? (
                ...
                paginatedItems.map((doc) => (
                  <tr key={doc.id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => setLocation(`/quotations/${doc.id}`)}>
                    <BulkSelectCell checked={bulk.selectedIds.has(doc.id)} disabled={!bulk.isSendable(doc)} onToggle={(checked) => bulk.toggleRow(doc.id, checked)} label={`Select ${doc.qtNumber}`} />
      ...
      <ListBulkEmailDialog
        ...
        docLabel="Quotations"
        numbers={bulk.selectedDocs.map(d => d.qtNumber)}
        generateAttachments={async () => {
          const attachments: { filename: string; content: string }[] = [];
          for (const doc of bulk.selectedDocs) {
            const content = await generateQuotation_PDF(doc, selectedCompany, settings as any, { returnBase64: true });
            if (typeof content !== "string" || !content) throw new Error(`Could not generate PDF for ${doc.qtNumber}.`);
            attachments.push({ filename: `${doc.qtNumber}.pdf`, content });
          }
          return attachments;
        }}
        onSuccess={async (recipients) => {
          await markDocsSent("quotations", bulk.selectedDocs.map(d => d.id), recipients);
