...
import { useGetSalesOrder, useUpdateSalesOrder, getGetSalesOrderQueryKey, useGetSettings, getGetSettingsQueryKey } from "@workspace/api-client-react";
...
export default function SalesOrderEdit() {
  ...
  const linkedQt = useRef<{ qtId: number | null; qtNumber: string | null }>({ qtId: null, qtNumber: null });
  ...
  const { data: doc } = useGetSalesOrder(id, {
  ...
  useEffect(() => {
    if (doc && !initialized.current) {
      ...
      // restored when the user comes back to edit a SalesOrder with a discount.
      ...
      linkedQt.current = { qtId: (doc as any).qtId ?? null, qtNumber: (doc as any).qtNumber ?? null };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => setLocation(`/sales-orders/${id}`)}><ArrowLeft className="h-4 w-4" /></Button>
          <div>
