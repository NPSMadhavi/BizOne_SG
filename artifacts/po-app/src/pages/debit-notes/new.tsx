...
export default function DebitNoteNew() {
  ...
  async function doSubmit(status: "draft" | "confirmed") {
    ...
    setSubmitting(true);
    try {
      const r = await fetch("/api/debit-notes", {
    ...
    <div className="max-w-[1600px] mx-auto pb-20 space-y-6 animate-in fade-in duration-300">
      ...
      {savedDoc && showPreview && (
        <PdfPreviewModal
          ...
          defaultEmailSubject={`Debit Note ${savedDoc.dnNumber}`}
          defaultEmailBody={`Dear ${(savedDoc as any).contactPerson || "Sir/Madam"},\n\nPlease find attached Debit Note ${savedDoc.dnNumber}.\n\nThank you.`}
          docInfo={{
            docType: "Debit Note",
            docNumber: savedDoc.dnNumber,
          ...
          onEdit={() => setLocation(`/debit-notes/${savedDoc.id}/edit`)}
        />
      )}
    </div>
  );
