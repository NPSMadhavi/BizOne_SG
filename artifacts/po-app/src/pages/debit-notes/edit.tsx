...
export default function DebitNoteEdit() {
  ...
  async function doSubmit(status: "draft" | "confirmed") {
    ...
    setSubmitting(true);
    try {
      const r = await fetch(`/api/debit-notes/${id}`, {
  ...
  if (doc?.status === "void") return (
    <div className="text-center py-16">
      ...
      <Link href={`/debit-notes/${id}`}><Button variant="outline" className="mt-4">View Debit Note</Button></Link>
    </div>
  );
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-20 space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between pb-4 border-b border-gray-200">
        <div>
          <Link href={`/debit-notes/${id}`} className="text-xs text-gray-400 hover:text-gray-600 flex items-center gap-1 mb-2">
            <ArrowLeft className="h-3 w-3" />Back
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">Edit Debit Note — {doc?.dnNumber}</h1>
        </div>
              ...
              <div className="flex justify-end p-4 border-t border-gray-200">
                <div className="w-72 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between font-bold text-base border-t pt-2 text-gray-900">
                    <span>Debit Total</span><span className="font-mono">{currency} {fmt(total)}</span>
                  </div>
                </div>
              </div>
            </CardContent>
      ...
      {showPreview && (savedDoc || doc) && (
        <PdfPreviewModal
          open={showPreview}
          onOpenChange={(open) => { if (!open) { setShowPreview(false); setLocation(`/debit-notes/${id}`); } }}
          title={(savedDoc ?? doc).dnNumber}
          generatePdf={(opts) => generateDebitNote_PDF(savedDoc ?? doc, selectedCompany, opts)}
          pdfFilename={`${(savedDoc ?? doc).dnNumber}.pdf`}
          defaultEmailTo={(savedDoc ?? doc as any).contactEmail || ""}
          defaultEmailSubject={`Debit Note ${(savedDoc ?? doc).dnNumber}`}
          defaultEmailBody={`Dear ${(savedDoc ?? doc as any).contactPerson || "Sir/Madam"},\n\nPlease find attached Debit Note ${(savedDoc ?? doc).dnNumber}.\n\nThank you.`}
          docInfo={{
            docType: "Debit Note",
            docNumber: (savedDoc ?? doc).dnNumber,
