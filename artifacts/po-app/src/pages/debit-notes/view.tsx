...
export default function DebitNoteView() {
  ...
  const { data: doc, isLoading, refetch } = useQuery<DebitNote>({
    queryKey: ["debit-note", id],
    queryFn: async () => {
      const r = await fetch(`/api/debit-notes/${id}`, { credentials: "include" });
  ...
  async function handleConfirm() {
    const r = await fetch(`/api/debit-notes/${id}/confirm`, { method: "POST", credentials: "include" });
  ...
  async function handleVoid() {
    const r = await fetch(`/api/debit-notes/${id}/void`, {
    ...
    <div className="max-w-7xl mx-auto pb-20 space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4 pb-4 border-b border-gray-200">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {(doc.status === "draft") && (
            <Button variant="outline" className="gap-2" onClick={() => setLocation(`/debit-notes/${id}/edit`)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm text-gray-600">Debit Note Details</CardTitle></CardHeader>
          <CardContent>
      ...
      <Dialog open={showVoid} onOpenChange={setShowVoid}>
        <DialogContent>
          <DialogHeader><DialogTitle className="flex items-center gap-2"><Ban className="h-5 w-5 text-red-600" />Void Debit Note</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-gray-600">Are you sure you want to void <strong>{doc.dnNumber}</strong>? This cannot be undone.</p>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleVoid}>Void Debit Note</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {showPreview && (
        <PdfPreviewModal
          ...
          title={doc.dnNumber}
          generatePdf={(opts) => generateDebitNote_PDF(doc as any, selectedCompany, opts)}
          pdfFilename={`${doc.dnNumber}.pdf`}
          defaultEmailTo={doc.contactEmail || ""}
          defaultEmailSubject={`Debit Note ${doc.dnNumber}`}
          defaultEmailBody={`Dear ${doc.contactPerson || "Sir/Madam"},\n\nPlease find attached Debit Note ${doc.dnNumber}.\n\nThank you.`}
          docInfo={{
            docType: "Debit Note",
            docNumber: doc.dnNumber,
          ...
          onEdit={doc.status === "draft" ? () => setLocation(`/debit-notes/${id}/edit`) : undefined}
        />
      )}
    </div>
  );
