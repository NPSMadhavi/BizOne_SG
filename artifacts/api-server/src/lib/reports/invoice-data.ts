...
export async function getInvoiceReportData(invoiceId: number, companyId: number): Promise<InvoiceReportPayload | null> {
  ...
  return {
    ...
    invoice: {
      ...
      customer_note: str((invoice as any).customerNote),
      delivery_instructions: str((invoice as any).deliveryInstructions),
      terms_and_conditions: str((invoice as any).termsAndConditions),
      authorised_signature: str((invoice as any).authorisedSignature),
